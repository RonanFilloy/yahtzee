...
class FullHouse extends Rule {
  // TODO
  evalRoll = dice => {
    const resultString = JSON.stringify(this.freq(dice));
    const fullHouse1 = JSON.stringify([2, 3]);
    const fullHouse2 = JSON.stringify([3, 2]);
    return resultString === fullHouse1 || resultString === fullHouse2 ? this.score : 0
  };
}
...
const ones = new TotalOneNumber({ val: 1, description: '1 point per each 1' });
const twos = new TotalOneNumber({ val: 2, description: '2 point per each 2' });
const threes = new TotalOneNumber({ val: 3, description: '3 point per each 3' });
const fours = new TotalOneNumber({ val: 4, description: '4 point per each 4' });
const fives = new TotalOneNumber({ val: 5, description: '5 point per each 5' });
const sixes = new TotalOneNumber({ val: 6, description: '6 point per each 6' });
